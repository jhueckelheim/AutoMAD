...
class DualTensor(torch.Tensor):
    ...
    TODO: This currently only supports 2d image data, but can be easily
    adjusted to higher dimensions.
    '''
    @staticmethod
...
class Fwd2Rev(torch.nn.Module):
    ...
    class __Func__(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            grad_input = input_d*grad_output.unsqueeze(2)
            return grad_input.view(input_d.size(0)*input_d.size(1),input_d.size(2),input_d.size(3),input_d.size(4))
    ...
    class __Func__(torch.autograd.Function):
        ...
        def forward(ctx, fwdinput_dual, weight, bias=None):
            if(isinstance(fwdinput_dual, DualTensor)):
                ...
                fwdinput_d = torch.Tensor(fwdinput_dual)
            else:
            ...
            ret_dual = DualTensor(ret_d, ret_p)
        ...
        def backward(ctx, grad_output):
            ...
            # We create a view to distinguish true batches from batches that
            # represent directional derivatives.
            grad_output = grad_output.view(n_batches, grad_output.size(0)//n_batches, grad_output.size(1), grad_output.size(2), grad_output.size(3))
            ...
            grad_input = grad_input.reshape(grad_input.size(0)*grad_input.size(1), grad_input.size(2), grad_input.size(3), grad_input.size(4))
            # grad_weight and grad_bias are the incoming grad_out, summed up
            # over the true channels.
            grad_weight = grad_output[:,-10:-1,:,:,:].sum(dim=[0,2,3,4]).reshape(1,1,3,3) if ctx.needs_input_grad[1] else None
            grad_bias = grad_output[:,-1:,:,:,:].sum(dim=[0,2,3,4]) if ctx.needs_input_grad[2] else None
