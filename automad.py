        ...
        def forward(ctx, fwdinput_dual, weight, bias, seedfunc, accmfunc, mode, args, kwargs):
            ...
            with torch.no_grad():
                if ctx.needs_input_grad[0]:
                    if(mode == 'layer-jacobian'):
                        fwdinput = fwdinput_dual
                        n_dervs_in, fwdinput_d = seedfunc(fwdinput.shape)
                    else:
                ...
                if(mode in ("jacobian", "layer-jacobian")):
        ...
        def forward(ctx, fwdinput_dual, weight, bias, seedfunc, accmfunc, mode, args, kwargs):
            ...
            with torch.no_grad():
                if ctx.needs_input_grad[0]:
                    ...
                    n_batch = fwdinput.size(0)
                    if(mode == 'layer-jacobian'):
                        fwdinput_d = seedfunc(fwdinput.shape)
                    else:
                        fwdinput_d = fwdinput_dual.xd
                    ret_d_in = ForwardUtilities.propagate(
                                   fwdinput, fwdinput_d, weight, None, bias, None,
...
class Rev2Fwd(torch.nn.Module):
    ...
    forward-mode AD together. It will allow the forward mode to compute the
    gradient of subsequent computations, and seed the backpropagation for the
    preceding layers with this gradient.
    '''
